...
def generationAccesstoken(datasetId, reportId, groupId):
    APPLICATION_ID = "c78e3b8b-5129-462e-aaee-6168d801d2b3"  # The ID of the application in Active Directory
    APPLICATION_SECRET = "sh58Q~IFGX66OIiA34Athu0lnZrXk8NJWhxnvbch"  # A valid key for that application in Active Directory

    USER_ID = "Uday@3th1rr.onmicrosoft.com"  # A user that has access to PowerBI and the application
    USER_PASSWORD = "Magneticme@0509"  # The password for that user
...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        queryUsername = "select * from tenant_details.test_tbl_user where username = '" + username.lower() + "'"
        cursor.execute(queryUsername)
        recordNew = cursor.fetchone()
        if record is None and recordNew is None:
            return Response({"message": "No User with Username"},
                            status=status.HTTP_400_BAD_REQUEST)
        if record is None:
            record = recordNew
        if not record[len(record) - 1]:
        ...
        if Insertedpassword == password:
            ...
            querytoken = f"select * from tenant_details.test_tbl_token where county_unique_id = '{record2[0]}'"
            cursor.execute(querytoken)
            databaseTokens =cursor.fetchone() 
            conn.commit()
            conn.close()
            BITokens = None
            MLTokens = None
            MarketingToken = None
            if querytoken is not None:
                # if (datetime.datetime.utcnow() - timedelta(minutes=60)) > databaseTokens[1]:
                if (datetime.datetime.utcnow()) - timedelta(minutes=60)  < databaseTokens[1]:
                    BITokens = [databaseTokens[2],databaseTokens[3]]
                    MLTokens = [databaseTokens[4],databaseTokens[5]]
                    MarketingToken = databaseTokens[6]
                else:
                    BITokens = tokensChoice(record2[0], "BI", "login")
                    # BITokenWeb = BITokens[0]
                    # BITokenMobile = BITokens[1]
                    MLTokens = tokensChoice(record2[0], "ML", "login")
                    # MLTokenWeb = MLTokens[0]
                    # MLTokenMobile = MLTokens[1]
                    MarketingToken = tokensChoice(record2[0], "market", "login")['token']
                    tokenInsertInDB(record2[0], BITokens + MLTokens + [MarketingToken])
            else:
                BITokens = tokensChoice(record2[0], "BI", "login")
                BITokenWeb = BITokens[0]
                BITokenMobile = BITokens[1]
 ...
 HAVING count(p."DimParcelPk") = 1;
    ...
    for i in ListSql:
        ...
        try:
            ...
            print("Error occurred in table ", str(e))
    conn.commit()
    conn.close()


def create_database(database_name):
    # HOST = 'https://aristanalytics.documents.azure.com:443/'
    # MASTER_KEY = 'g40T3IriQjf7YbUGmzms3XGAHpniZz5poWiSa4MNGCtDt9h6gdEhtV9cpwt9sv2vyyM57ZIqpG8QFAVOp7IduQ=='
    # MASTER_KEY = 'rpMU98iPMJGqRgMtJdQu9wyFTSbk20mH50NLC4yBwhbL56ZMAR6eeP9wWnzcuJni5irHIzihUaSrACDbDltEnQ=='
    HOST = 'https://arist.documents.azure.com/'
    MASTER_KEY = 'g40T3IriQjf7YbUGmzms3XGAHpniZz5poWiSa4MNGCtDt9h6gdEhtV9cpwt9sv2vyyM57ZIqpG8QFAVOp7IduQ=='
...
def run(stmt):
    # cur = 
    # psycopg2.connect(database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
...
def PowerBiAutoMation(name):
    ...
    groupIdForClone = "3b4f66c0-3d4a-4453-ac1c-069498bce767"
    ...
    reportIdWeb = "97ffacd3-839b-4e7c-b3e6-7febfbde83fa"
    reportIdMobile = "2a970d77-60c4-41a7-9301-93b751e8c801"
    ...
    records.append("https://app.powerbi.com/reportEmbed?reportId=" + reportCloneWeb.json()[
        "id"] + "&autoAuth=true&ctid=da009663-27a1-415b-9f30-97718a8c769c")  # Adding I Frame URL
    ...
    records.append("https://app.powerbi.com/reportEmbed?reportId=" + createReportCloneMobile.json()[
        "id"] + "&autoAuth=true&ctid=da009663-27a1-415b-9f30-97718a8c769c")  # Adding I Frame URL mobile
...
def UpdateDB(uniqueId, recordsML, recordsBI):
    # conn = psycopg2.connect(
    #     database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
    #     host='aristanalyticspoc.postgres.database.azure.com', port='5432'
    # )
...
def MLModel(countyName):
    ...
    source_group_id = "45a32a70-05c7-4885-8b9c-14d4a26450b4"
    source_report_id = "822ae148-619d-4c84-bfc4-01f1345b4699"
    source_mobile_report_id = "9e8701a6-d0c0-4535-b2e3-efdbbe4f7041"
    ...
    iframe1 = "https://app.powerbi.com/reportEmbed?reportId=" + reportClone.json()[
        "id"] + "&autoAuth=true&ctid=da009663-27a1-415b-9f30-97718a8c769c"
...
def AutomationProcessBIAndML():
    ...
    for i in record:
        if (datetime.datetime.utcnow()) - timedelta(minutes=30) > i[4]:
            recordsBI = PowerBiAutoMation(i[2])
            try:
                recordsML = MLModel(i[2])
                UpdateDB(i[0], recordsML, recordsBI)
            except Exception as e:
...
def tokensChoice(uniqueId, choice, usage='token'):
    tokens = None
    if usage == 'login':
        print("In Login")
        ...
        schemaSet = "set schema 'tenant_details'"
        cursor.execute(schemaSet)
        query1 = "select * from tenant_details.test_tbl_token where county_unique_id = '" + uniqueId + "'"
        print(query1)
        cursor.execute(query1)
        print("test 1 ")
        ...
        conn.close()
        print(records)
        if (datetime.datetime.utcnow()) - timedelta(minutes=60) > records[1]:
            print("Update check 1")
            tokens = GenerateTokens(uniqueId, choice)
            print(tokens)
            tokenUpdateInDB(uniqueId, tokens, choice.lower())
            print("at return statement")
            return tokens
        else:
            if choice.lower() == 'ml':
                tokens = [records[4], records[5]]
                # tokenUpdateInDB(uniqueId,tokens,'ml')
            elif choice.lower() == 'bi':
                tokens = [records[2], records[3]]
            elif choice.lower() == 'market':
                tokens = records[6]
                # tokenUpdateInDB(uniqueId, tokens, 'bi')
    return tokens


def tokenInsertInDB(uniqueId, value):
    # conn = psycopg2.connect(
    #     database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
    #     host='aristanalyticspoc.postgres.database.azure.com', port='5432'
    # )
    ...
    cursor = conn.cursor()
    checkQuery = " select * from tenant_details.test_tbl_token where county_unique_id ='" + uniqueId + "'"
    cursor.execute(checkQuery)
    check = cursor.fetchone()
    print(check)
    if check is None:
        print("In if Block ")
        query1 = ''' INSERT INTO tenant_details.test_tbl_token(
        county_unique_id, created_at, bi_token_web, bi_token_mobile, ml_token_web, ml_token_mobile,marketing_token)
        VALUES (%s, %s, %s, %s, %s, %s,%s);'''
        record_to_insert1 = (uniqueId, datetime.datetime.utcnow(),
                             value[0],
                             value[1],
                             value[2],
                             value[3], value[4])
        ...
        print("In else block ")
        query2 = '''
        UPDATE tenant_details.test_tbl_token
	    SET  created_at=%s, bi_token_web=%s, bi_token_mobile=%s, ml_token_web=%s, ml_token_mobile=%s,marketing_token=%s
	    WHERE county_unique_id= %s ;
        '''
        record_to_insert2 = (datetime.datetime.utcnow(),
                             value[0],
                             value[1],
                             value[2],
                             value[3],
...
def tokenUpdateInDB(uniqueId, value, choice):
    print("In tokenUpdateInDB method ")
    # conn = psycopg2.connect(
    #     database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
    #     host='aristanalyticspoc.postgres.database.azure.com', port='5432'
    # )
...
# This Function is used to generate the Tokens for Power BI and ML Dashboard
def GenerateTokens(uniqueId, choice):
    ...
    if choice.lower() == 'bi':
        ...
        if record2[0] == '638FOC1YHDCQ7OB':  # Rutherford Mobile report Id
            datasetIdMobile = "990fd67c-ee65-42a3-a070-f55b24ba5d3e"
            # datasetIdMobile = "4d8ae096-5dda-4c03-9971-f621f1c733c7"
        if record2[0] == 'QGX5R1C1UR5A03U':  # Beauford Mobile report Id
            datasetIdMobile = "1a7e3917-ff5e-48c8-8f2c-75f93bfb6e1a"
        ...
        try:
            ...
            print("issue with mobile token is ", str(e))
        v = [BITokenWeb, BITokenMobile]
        return v
    if choice.lower() == 'market':
        datasetId = record2[20]  # markting feilds 17,18,19 ,20
        reportId = record2[19]
        groupId = record2[18]
        accessToken = generationAccesstoken(datasetId, reportId, groupId)
        return accessToken
    else:
        return None
...
class AccessTokenForMLAndBI(APIView):
    def post(self, request):
        try:
            ...
            tokens = tokensChoice(uniqueId, choice)
            if (tokens != None):
                if choice.lower() == 'market':
                    return Response({"MarketToken": tokens}, status=status.HTTP_200_OK)
                return Response({"WebToken": tokens[0], "MobileToken": tokens[1]}, status=status.HTTP_200_OK)
            else:
                return Response({"message": "Wrong Choice Selected "}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({"message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
