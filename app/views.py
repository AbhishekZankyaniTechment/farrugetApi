...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        cursor = conn.cursor()
        schemaSet = "set schema 'test_tenant_details'"
        cursor.execute(schemaSet)
        query1 = "select * from test_tenant_details.test_tbl_user where county_name = '" + username.lower() + "'"
...
def InsertIntoDataBaseCountyDetails(records, username, encPassword, countyName, uniqueId):
    ...
    schemaSet = "set schema 'test_tenant_details'"
    cursor.execute(schemaSet)
    query1 = ''' INSERT INTO test_tenant_details.test_tbl_user( county_unique_id, username, county_name, password, created_at, active)
    	VALUES (%s, %s, %s, %s,%s,%s);'''
    record_to_insert1 = (uniqueId, username, countyName, encPassword, dt, False)
    cursor.execute(query1, record_to_insert1)
    query2 = ''' INSERT INTO test_tenant_details.test_tbl_county_details(
...
def getAllCounties():
    ...
    # Creating a cursor object using the cursor() method
    cursor = conn.cursor()
    string = "SET SCHEMA 'test_tenant_details'"
    cursor.execute(string)
...
def UpdateDB(uniqueId, recordsML, recordsBI):
    ...
    query1 = ''' UPDATE test_tenant_details.test_tbl_county_details
	             SET group_id_ml=%s,
	             dataset_id_ml=%s,
	             report_id_ml_web=%s,
...
def AutomationProcessBIAndML():
    ...
    cursor = conn.cursor()
    query1 = 'select * from test_tenant_details.test_tbl_user where active=false '
    cursor.execute(query1)
...
def GenerateTokens(uniqueId, choice):
    ...
    schemaSet = "set schema 'test_tenant_details'"
    cursor.execute(schemaSet)
    query1 = "select * from test_tenant_details.test_tbl_county_details where unique_id = '" + uniqueId + "'"
    cursor.execute(query1)
    record2 = cursor.fetchone()
    conn.commit()
    conn.close()
